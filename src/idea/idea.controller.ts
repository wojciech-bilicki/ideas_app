...
export class IdeaController {
  ...
  private logData(options: any) {
    // @ts-ignore
    if (options.data) { this.logger.log('DATA' + JSON.stringify(options.body)); }
    if (options.user) { this.logger.log('USER' + JSON.stringify(options.user)); }
    if (options.id) { this.logger.log('IDEA' + JSON.stringify(options.id)); }
  }
